...
/** Mancha simétrica estilo Rorschach — loading de Clínicas. */
export function ClinicRorschachLoader({
  label = "Carregando clínicas…",
}: {
  label?: string;
}) {
  return (
    <div
      className="clinic-shell clinic-loading fixed inset-0 z-[100] flex flex-col items-center justify-center gap-7 px-6 font-[family-name:var(--font-clinic)]"
      style={{ colorScheme: "light" }}
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
      <div className="clinic-rorschach" aria-hidden="true">
        <svg
          viewBox="0 0 240 240"
          className="w-40 h-40 sm:w-48 sm:h-48"
          fill="currentColor"
        >
          {/* Corpo central simétrico */}
          ...
          <path
            ...
            opacity="0.7"
          />
          {/* Detalhes internos (olhos / vazios) */}
      ...
      <p className="text-sm font-medium tracking-wide text-[var(--clinic-muted)]">
        {label}
      </p>
    </div>
  );
